...
def setup(step, config):
    if step == 1:
        ...
        fm = open(local_work_dir + '/.' + deployment_id + '_mesos_masters_ips', 'w')
        for ip in master_ips:
            f.write(ip + "\n")
            fm.write(ip + "\n")

        print("==> Writing mesos slaves ips in " + local_work_dir + "/" + deployment_id + "_.mesos_slaves_ips files")
        slaves_ips = setup_helpers.get_slave_instances_ips(deployment_id, config)
        fs = open(local_work_dir + '/.' + deployment_id + '_mesos_slaves_ips', 'w')
    ...
    if args.clean:
        ...
        for step in range(start_step, end_step + 1):
            ...
            if step == default_end_step:
                # Remove step file
                print("Going to remove step file %s" % step_file)
                cmd = "rm " + step_file
                shell_call(cmd)
